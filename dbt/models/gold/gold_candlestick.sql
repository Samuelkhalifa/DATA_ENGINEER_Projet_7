...
enriched AS (
    SELECT
        ...
        day_high,
        current_price,
        FIRST_VALUE(current_price) OVER (PARTITION BY symbol, CAST(market_timestamp AS DATE) ORDER BY market_timestamp) AS candle_open,
        LAST_VALUE(current_price) OVER (PARTITION BY symbol, CAST(market_timestamp AS DATE) ORDER BY market_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS candle_close
